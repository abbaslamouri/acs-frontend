...
const { fetchAll, productsFetchAll, deleteDoc, deleteDocs } = useHttp()

const quantitySelectors = ref([])
const listType = ref('tile')

const showProductFiltersSlideout = ref(true)
...
const showSearchResults = async (serachObject) => {
  console.log(serachObject)
  response = await productsFetchAll(serachObject)
  console.log(response)
  if (!response) return
  products.value = response.docs
  ...
  //   const query = Object.keys(serachObject)
  //     .map((k) => esc(k) + '=' + esc(serachObject[k]))
  //     .join('&')
